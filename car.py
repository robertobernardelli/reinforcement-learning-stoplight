...
class Car:
    def __init__(self, path, id):
        ...
        self.next_node.queue.add(self) #when initializing, add the car to the queue of the next node
        self.distance = self.determine_distance()
        self.direction = self.get_direction()
        self.time = 0 #time spent on this earth
        
        #we assume that the speed and acceleration of the car are normally distributed and have 5% standard deviation
    ...
    def determine_distance(self):
        #if the next node is a stoplight and it is red, or if the next node is a yield node and it is occupied
    ...
    def step(self):
        ...
        if self.angle_between_vectors(self.next_node.pos - self.pos, self.direction) < 0.7:
            ...
            #if the new node is a ListNode, remove the car from the old list and add it to the new list
        ...
        #kill, remove from list and return time spent on this earth
